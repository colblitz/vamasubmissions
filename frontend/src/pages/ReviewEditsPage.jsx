          ...
          {activeTab === 'pending' ? (
            <>
              {!pendingEdits || pendingEdits.length === 0 ? (
                ...
                pendingEdits.map((edit) => (
                  <div key={edit.id} className="bg-white rounded-lg shadow p-4 flex gap-4 items-start">
                    {/* Thumbnail */}
                    {edit.post_thumbnail ? (
                      <img 
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className={`text-sm font-medium ${edit.action === 'ADD' ? 'text-green-700' : 'text-red-700'}`}>
                          {edit.value}
                        </span>
                      </div>
            ...
            <>
              {!history || history.length === 0 ? (
                ...
                history.map((item) => (
                  <div key={item.id} className="bg-white rounded-lg shadow p-4 flex gap-4 items-start">
                    {/* Thumbnail */}
                    {item.post_thumbnail ? (
                      <img 
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className={`text-sm font-medium ${item.action === 'ADD' ? 'text-green-700' : 'text-red-700'}`}>
                          {item.value}
                        </span>
                      </div>
                      <p className="text-xs text-gray-500 mt-1">
                        Approved by {item.approver_username} • {new Date(item.applied_at).toLocaleDateString()}
                      </p>
                    </div>
