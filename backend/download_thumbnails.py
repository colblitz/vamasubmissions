...
def download_thumbnails(json_dir: Path, output_dir: Path, dry_run: bool = False):
    ...
    for i, json_file in enumerate(json_files, 1):
        ...
        if existing_files:
            print(
                f"[{i}/{len(json_files)}] SKIP {post_id} (already exists: {existing_files[0].name})"
            )
            skipped += 1
            continue
        ...
        try:
            ...
            print(
                f"[{i}/{len(json_files)}] DOWNLOADED {post_id} → {output_file.name} ({file_size:.1f} KB)"
            )
            downloaded += 1
...
Examples:
  ...
  python3 download_thumbnails.py --json-dir /path/to/json --output-dir /path/to/output
    ...
    parser.add_argument("--dry-run", action="store_true", help="Test without actually downloading")
