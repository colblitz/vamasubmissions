...
class AliasCache:
    ...
    def reload_cache(cls, db: Session):
        ...
        cls._cache = {"characters": {}, "series": {}, "tags": {}}
...
def get_aliases(
    ...
    return ValueAliasList(aliases=aliases, total=len(aliases))
...
def get_zero_result_suggestions(
    ...
    suggestions = [
        ZeroResultSuggestion(
            field_type=row[0], search_term=row[1], search_count=row[2], last_searched=row[3]
        )
        for row in results
    ]

    return ZeroResultSuggestionList(suggestions=suggestions, total=len(suggestions))
