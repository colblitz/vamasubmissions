...
def get_next_queue_position(db: Session, queue_type: str) -> int:
    ...
    """
    max_position = (
        db.query(func.max(Submission.queue_position))
        .filter(Submission.queue_type == queue_type, Submission.status == "pending")
        .scalar()
    )
...
def reorder_queue(db: Session, queue_type: str) -> None:
    ...
    for idx, submission in enumerate(submissions, start=1):
        ...
        submission.estimated_completion_date = calculate_estimated_completion(db, queue_type, idx)
...
def delete_submission_images(db: Session, submission_id: int) -> None:
    ...
    Args:
        ...
        submission_id: Submission ID
    """
    images = db.query(SubmissionImage).filter(SubmissionImage.submission_id == submission_id).all()
...
def search_submissions(
    ...
    """
    search_query = db.query(Submission).filter(Submission.status == status)
...
def get_series_autocomplete(db: Session, query: str, limit: int = 10) -> List[str]:
    ...
    results = (
        db.query(Submission.series)
        .filter(Submission.series.ilike(search_pattern))
        .distinct()
        .limit(limit)
        .all()
    )
