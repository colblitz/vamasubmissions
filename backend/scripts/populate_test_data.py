...
from app.models import EditHistory, Post, PostEdit, User
...
def create_test_edits(session, posts: list, test_user: User, count: int = 10) -> list:
    ...
    print(
        f"Created {len(edits)} test edits ({pending_count} pending, {approved_count} approved, {rejected_count} rejected)"
    )
    return edits
...
def create_test_edit_history(session, edits: list, test_user: User) -> list:
    ...
    print(
        f"Created {len(history_entries)} test edit history entries ({approved_count} applied, {rejected_count} rejected)"
    )
